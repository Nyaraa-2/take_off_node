...
export const GET_USERS_DETAILS =
  ...
  '(SELECT JSON_AGG(ec.name) FROM skill AS s JOIN expertise_category AS ec ON s.id_expertise_category = ec.id JOIN account AS a2 ON s.id_account = a2.id_job  WHERE ec.id_expertise=1 AND a2.id=a.id) AS languages,' +
  '(SELECT JSON_AGG(ec2.name) FROM skill AS s JOIN expertise_category AS ec2 ON s.id_expertise_category = ec2.id JOIN account AS a3 ON s.id_account = a3.id  WHERE ec2.id_expertise=2 AND a3.id=a.id) AS frameworks ' +
